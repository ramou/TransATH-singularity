...
my $sfile = 'db/tcdb';
my $cfile = 'subcat';
my $blst = 'result/blast.out';
$hitsout = 'result/hits.out';
...
system("blastall -p blastp -d $sfile -i $qfile -e 0.00000000000000000001 -o $blst"); #-m 7 
...
my $blast_report = new Bio::SearchIO ('-format' => 'blast', '-file' => 'result/blast.out', -best_hit_only =>'true'); #-signif => '1e-5'

open (HITSOUT, ">$hitsout") or die "couldn't open $hitsout, $!\n";
print HITSOUT "Query\tHit\tTCID\tQry_Description\tHit_Description\tQry_TMS\tHit_TMS\tQry_Length\tSub_Length\thsp_hit-length\thsp_total-length\tQry_Region\tHit_Region\tPercent_Aln.\tQcov\tScov\tE-value\tScore\tSub_group\tspec_subs\n";

sub topology {
	my $seq = shift(@_);
	my $hmmtop = `/home/ijah/A-Works/TransportersX/gBlast/hmmtop -if=$seq`;
	...
	while( my $hit = $result->next_hit()) {   
		...
		while ($cnt < 5) { # just one loop to print out the top hits only
			...
			while( my $hsp = $hit->next_hsp()) {  
				...
				$percent = sprintf("%.2f", ($hsp->length('hit')/$hit->length)*100); #calculation for % alignment
				...
				$temp_query = 'result/temp_qry_faa';
				...
				$subj_seq = '>'.$hitdescs."\n".$shash{$tc_acc};
				$temp_subject = 'result/temp_subj_faa';
				...
				#$subg='';
				#$subs='';
				#foreach $i(@tid.$i){print $tid[0],"\t",$subg[0],"\n";
				#for ($j=0; $j<=$i; $j++){
				#print $tid.$j;} 					
					#if($tid eq $tcid){
					#	$subg = $subg[1];
				...
				#check subject topology		#topology("result/temp_subj_faa");
				#$tcid2='x'.$tcid.'x';
...
system("./gblast2.php $hitsout $qfile './result/clean.tsv'");

unlink ("result/temp_subj_faa");
unlink ("result/temp_qry_faa");
print "\n******* All Done! *******\n";
